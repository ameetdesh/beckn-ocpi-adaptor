...
interface RouteHandlerDependencies {
    appConfig: AppConfig;
...
export const createRouteHandler = (config: RouteHandlerConfig, dependencies: RouteHandlerDependencies) => {
    const { action, stage } = config;
    const { appConfig, insertLog } = dependencies;
